...
pub struct AutocadMcpServer {
    #[allow(dead_code)] // rmcp マクロが内部で使用
...
impl AutocadMcpServer {
    ...
    #[tool(
        name = "eval_lisp",
        description = "AutoLISP 式を評価（全機能アクセス可能）。信頼された環境専用。任意の AutoLISP を実行可能なため、入力の検証は呼び出し側の責任"
    )]
    ...
    #[tool(
        name = "acad_command",
        description = "AutoCAD コマンドを LISP 経由で実行。信頼された環境専用。任意の AutoLISP を実行可能なため、入力の検証は呼び出し側の責任"
    )]
    ...
    async fn get_bounds(&self, Parameters(_): Parameters<EmptyInput>) -> String {
        call("get_bounds", json!({}))
    }

    #[tool(name = "count_entities", description = "エンティティ数を取得")]
    async fn count_entities(&self, Parameters(_): Parameters<EmptyInput>) -> String {
        call("count_entities", json!({}))
    }

    // --- レイヤー ---
    #[tool(
        name = "list_layers",
        description = "レイヤー一覧（名前・色・表示状態）"
    )]
    ...
    async fn create_layer(&self, Parameters(i): Parameters<CreateLayerInput>) -> String {
        call(
            "create_layer",
            json!({"name": i.name, "color": i.color.unwrap_or(7)}),
        )
    }

    #[tool(
        name = "set_layer_property",
        description = "レイヤーのプロパティを変更"
    )]
    async fn set_layer_property(&self, Parameters(i): Parameters<SetLayerPropertyInput>) -> String {
        ...
        if let Some(o) = i.off {
            p["off"] = json!(o.to_string());
        }
        if let Some(l) = i.locked {
            p["locked"] = json!(l.to_string());
        }
    ...
    async fn draw_line(&self, Parameters(i): Parameters<DrawLineInput>) -> String {
        call(
            "draw_line",
            with_layer(
                json!({"x1":i.x1,"y1":i.y1,"z1":i.z1.unwrap_or(0.0),"x2":i.x2,"y2":i.y2,"z2":i.z2.unwrap_or(0.0)}),
                &i.layer,
            ),
        )
    }
    ...
    async fn draw_circle(&self, Parameters(i): Parameters<DrawCircleInput>) -> String {
        call(
            "draw_circle",
            with_layer(
                json!({"cx":i.cx,"cy":i.cy,"cz":i.cz.unwrap_or(0.0),"radius":i.radius}),
                &i.layer,
            ),
        )
    }
    ...
    async fn draw_arc(&self, Parameters(i): Parameters<DrawArcInput>) -> String {
        call(
            "draw_arc",
            with_layer(
                json!({"cx":i.cx,"cy":i.cy,"cz":i.cz.unwrap_or(0.0),"radius":i.radius,"start_angle":i.start_angle,"end_angle":i.end_angle}),
                &i.layer,
            ),
        )
    }
    ...
    async fn draw_polyline(&self, Parameters(i): Parameters<DrawPolylineInput>) -> String {
        ...
        call(
            "draw_polyline",
            with_layer(
                json!({"points": pts, "closed": i.closed.unwrap_or(false)}),
                &i.layer,
            ),
        )
    }
    ...
    async fn draw_text(&self, Parameters(i): Parameters<DrawTextInput>) -> String {
        call(
            "draw_text",
            with_layer(
                json!({"x":i.x,"y":i.y,"z":i.z.unwrap_or(0.0),"text":i.text,"height":i.height.unwrap_or(2.5),"rotation":i.rotation.unwrap_or(0.0)}),
                &i.layer,
            ),
        )
    }
    ...
    async fn draw_rectangle(&self, Parameters(i): Parameters<DrawRectangleInput>) -> String {
        ...
        call(
            "draw_polyline",
            with_layer(json!({"points": pts, "closed": true}), &i.layer),
        )
    }
    ...
    async fn move_entity(&self, Parameters(i): Parameters<MoveEntityInput>) -> String {
        call(
            "move_entity",
            json!({"entity_id":i.entity_id,"dx":i.dx,"dy":i.dy,"dz":i.dz.unwrap_or(0.0)}),
        )
    }
    ...
    async fn copy_entity(&self, Parameters(i): Parameters<CopyEntityInput>) -> String {
        call(
            "copy_entity",
            json!({"entity_id":i.entity_id,"dx":i.dx.unwrap_or(0.0),"dy":i.dy.unwrap_or(0.0),"dz":i.dz.unwrap_or(0.0)}),
        )
    }

    #[tool(
        name = "rotate_entity",
        description = "エンティティを回転（角度は度数）"
    )]
    async fn rotate_entity(&self, Parameters(i): Parameters<RotateEntityInput>) -> String {
        call(
            "rotate_entity",
            json!({"entity_id":i.entity_id,"cx":i.cx,"cy":i.cy,"angle":i.angle}),
        )
    }
    ...
    async fn scale_entity(&self, Parameters(i): Parameters<ScaleEntityInput>) -> String {
        call(
            "scale_entity",
            json!({"entity_id":i.entity_id,"cx":i.cx,"cy":i.cy,"factor":i.factor}),
        )
    }

    #[tool(
        name = "set_entity_layer",
        description = "エンティティのレイヤーを変更"
    )]
    async fn set_entity_layer(&self, Parameters(i): Parameters<SetEntityLayerInput>) -> String {
        call(
            "set_entity_layer",
            json!({"entity_id":i.entity_id,"layer":i.layer}),
        )
    }
    ...
    async fn set_entity_color(&self, Parameters(i): Parameters<SetEntityColorInput>) -> String {
        call(
            "set_entity_color",
            json!({"entity_id":i.entity_id,"color":i.color}),
        )
    }
    ...
    fn get_info(&self) -> ServerInfo {
        ...
        info.instructions = Some(
            "AutoCAD MCP Server — 描画(line/circle/arc/polyline/text/rectangle)、\
             編集(move/copy/rotate/scale/delete)、レイヤー管理、クエリ、eval_lisp。"
                .into(),
        );
        info
    }
